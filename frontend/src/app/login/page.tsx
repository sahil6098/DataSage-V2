...
function UserIcon() {
  return (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
      <path
        d="M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zm-7 9a7 7 0 0 1 14 0"
        stroke="#2563eb"
        strokeWidth="2"
        strokeLinecap="round"
    ...
    <>
      ...
      <div className="di di-table" style={{ filter: "drop-shadow(0 8px 16px rgba(244, 63, 94, 0.4)) drop-shadow(0 0 12px rgba(147, 51, 234, 0.5))" }}>
        <svg viewBox="0 0 48 38" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect x="1.5" y="1.5" width="45" height="35" rx="3.5" stroke="url(#grad-table)" strokeWidth="2.5"/>
      ...
      <div className="di di-scatter" style={{ filter: "drop-shadow(0 8px 16px rgba(20, 184, 166, 0.4)) drop-shadow(0 0 12px rgba(234, 179, 8, 0.5))" }}>
        <svg viewBox="0 0 48 36" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="26" cy="30" r="3.5" fill="url(#grad-scatter)"/>
...
export default function LoginPage() {
  ...
  return (
    <>
      ...
      <main className="sp-shell" onPointerMove={handlePointerMove} onPointerLeave={handlePointerLeave}>
        {/* ── LEFT: Animated panel ── */}
        <div className="sp-left">
          ...
          <div className="sp-brand">
            <span className="sp-brand-icon"><BrandIcon /></span>
            DataSage AI
          </div>

          <div
            className={`sp-robot-wrap ${isAngry ? 'sp-angry' : ''} ${isFlying ? 'sp-flying' : ''}`}
          ...
          >
            ...
            <div ref={robotWrapRef} style={{ width: '100%', height: '100%', transition: 'transform 0.1s ease-out' }}>
              <div className={`sp-angry-inner ${isAngry ? 'angry-shake' : ''}`} style={{ width: '100%', height: '100%', position: 'relative' }}>
                <RiveComponent />
              </div>
            </div>
          </div>
          <div className={`sp-robot-shadow ${isFlying ? 'sp-shadow-shrink' : ''}`} />
        </div>

        {/* ── RIGHT: Form panel ── */}
        <div className="sp-right">
          <div className={`sp-card${shake ? ' sp-shake' : ''}`}>

            {/* Header */}
            <div className="sp-header">
              <div className="sp-avatar"><UserIcon /></div>
              <h1 className="sp-heading">Welcome back!</h1>
              <p className="sp-sub">Sign in to your DataSage AI workspace</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              {/* Email */}
              <div className={`sp-field${(emailFocused || email) ? ' sp-field--up' : ''}`}>
                <input
                  id="lg-email"
                  type="email"
                  placeholder=" "
              ...
              {/* Password */}
              <div className={`sp-field sp-field--pw${(pwFocused || password) ? ' sp-field--up' : ''}`}>
                <input
                  id="lg-password"
                ...
                <button type="button" className="sp-eye" onClick={() => setShowPw(s => !s)}>
                  {showPw ? <EyeOff size={17} /> : <Eye size={17} />}
                </button>
              </div>

              {/* Forgot */}
              <div className="sp-forgot-row">
                <a href="#" className="sp-forgot">Forgot password?</a>
              </div>
            ...
            {/* Google */}
            <button className="sp-btn-google">
            ...
            <p className="sp-switch">
              ...
              <Link href={toAppPath('/register', pathname)} className="sp-switch-link">
                Register →
              </Link>
            </p>
          </div>
...
.sp-robot-wrap.sp-flying {
  animation: robot-fly 2s cubic-bezier(0.3, 0, 0.2, 1) forwards;
}
...
.sp-robot-shadow.sp-shadow-shrink {
  animation: shadow-shrink 2s cubic-bezier(0.3, 0, 0.2, 1) forwards;
}
...
.sp-robot-wrap.sp-angry {
  /* A powerful angry reaction using CSS */
...
.sp-angry-inner.angry-shake {
  animation: robot-angry-shake 0.3s cubic-bezier(.36, .07, .19, .97) both infinite;
}

/* Pseudo angry eyebrows overlaid directly on face area */
.sp-angry-inner.angry-shake::after {
  content: "";
  position: absolute;
  top: 25%;  
  left: 38%;
  ...
  pointer-events: none;
  animation: eyebrow-slam 0.2s forwards cubic-bezier(0.6, -0.4, 0.2, 1.5);
  transform-origin: top center;
  z-index: 10;
}

@keyframes eyebrow-slam {
  0% { transform: translateY(-40px) scale(1.2); opacity: 0; }
  100% { transform: translateY(0px) scale(1); opacity: 1; }
}

@keyframes robot-angry-shake {
...
.sp-right {
  ...
  justify-content: center;
  padding: 48px 44px;
  box-shadow: -8px 0 48px rgba(0, 0, 0, 0.07);
  overflow-y: auto;
}
...
.sp-card {
  width: 100%;
  max-width: 380px;
  animation: spFadeUp 0.55s cubic-bezier(0.22, 1, 0.36, 1) both;
}
...
.sp-shake { animation: spShake 0.55s ease; }
...
.sp-field--pw input { padding-right: 48px; }
...
.sp-switch-link:hover { text-decoration: underline; }

/* ── Responsive ── */
@media (max-width: 900px) {
  .sp-left  { display: none; }
  .sp-right { width: 100%; padding: 36px 24px; }
}
`;
